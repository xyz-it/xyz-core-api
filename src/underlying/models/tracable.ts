 ...
 * @private
 */
function _getter_setter_factory_(key: string, valueType: string) {

    /*
        This weak map stores attributes for any objects. Key is the object instance itself
     */
    let privateValue: WeakMap<any, any> = new WeakMap<any, any>();


    /*
        Some implicit conversions may be used
     */
    let conv = conversion.from("string").to(valueType);
    ...
    if (valueType === "string") {
        setter = function (val: any) {
            let oldValue = privateValue.get(this);
        ...
        setter = function (val: any) {
            ...
            if (targetVal && targetVal._isEnhanced) {
                let keys = Object.getOwnPropertyNames(targetVal);
                for (let idx in keys) {
                    let subkey = keys[idx];
...
export function enhancePropertiesOf<T>(target: T ): T {
    ...
    let targetedObject = _.isPlainObject(target) ? target : target.constructor.prototype;
    ...
    for (let key in target) {
        if (target.hasOwnProperty(key) && !key.startsWith("_") && key !== "id" && key !== "documentId" && key !== "itemId" && key !== "scheduleId") {
            ...
            let valueType:string = _.isDate(value) ? "date" : typeof value;

            if (Reflect.deleteProperty(targetedObject, key)) {

                let factory = _getter_setter_factory_(key, valueType);
